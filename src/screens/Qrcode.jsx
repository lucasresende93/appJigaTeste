...
const Qrcode = ({ navigation }) => {
  ...
  const sendToServer = async (data) => {
    ...
    try {
      ...
      if (error.response.status === 400) {
        Alert.alert("Error",
          `Peça ${data.serialNumber}${data.index} já esta cadastrada no servidor.`)
      }
    }
  }
  ...
  useEffect(() => {
    ...
    let messageCrc = (((0xff - (sumTotal & 0xff)) + 1) & 0xff);

    console.log('sumTotal', Number(sumTotal), sumTotal.toString(16));
    console.log('messageCrc', messageCrc.toString(16));
